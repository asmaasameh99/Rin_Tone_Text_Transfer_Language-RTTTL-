...
for i=1:l ;  % for loop for each element in cell y 
    if y{i}(1)=='1'; %check the duration
        if y{i}(2)=='6';
            D=16 ;
            p=3; %Pointer to point at the certain index
        else 
            D=1;
            p=2;
        end
    ...
    p=p+1; % increase the pointer by 1 to check the next index  
    if y{i}(p)=='#' ;
        sharp='#';
        note=[note sharp];
    ...
    p=p+1;% increase the pointer by 1 to check the next index  
    if y{i}(p)=='.';
        D=1.5*D; % the duartion become 1.5 from default
        t=0:1/fs:(dw/D);
    end 
        if y{i}(p) == y{i}(end)
            if      o==4
    ...
    elseif note=='g#'
      ...
      elseif note=='p'
        ...
        final{1,i}=[n];
        continue
        else p=p+1; % increase the pointer by 1 to check the next index  
    end
